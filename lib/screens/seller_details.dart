...
class _SellerDetailsState extends State<SellerDetails> {
  ...
  Future checkFollowed() async {
    if (SystemConfig.systemUser != null && SystemConfig.systemUser!.id != null) {
  ...
  fetchNewArrivalProducts() async {
    var newArrivalProductResponse = await ShopRepository().getNewFromThisSellerProducts(id: _shopDetails?.id);
  ...
  fetchTopProducts() async {
    var topProductResponse = await ShopRepository().getTopFromThisSellerProducts(id: _shopDetails?.id);
    _topProducts.addAll(topProductResponse.products!);
    _topProductInit = true;
  }

  fetchFeaturedProducts() async {
    var featuredProductResponse = await ShopRepository().getfeaturedFromThisSellerProducts(id: _shopDetails?.id);
              ...
              SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  Padding(
                    ...
                    child: _shopDetails == null ? buildShopDetailsShimmer() : buildShopDetails(),
                  ),
                  Padding(
                    padding: const EdgeInsets.fromLTRB(18.0, 20.0, 18.0, 0.0),
                    child: _shopDetails == null ? buildTabOptionShimmer(context) : buildTabOption(context),
                  ),
  ...
  Widget buildTabBarBody(BuildContext context) {
    if (tabOptionIndex == 1) {
      return _shopDetails != null ? buildTopSelling(context) : ShimmerHelper().buildProductGridShimmer();
    }
    if (tabOptionIndex == 2) {
      return _shopDetails != null ? buildAllProducts(context) : ShimmerHelper().buildProductGridShimmer();
    }

    return _shopDetails != null ? buildStoreHome(context) : buildStoreHomeShimmer(context);
  }
    ...
    return AnimatedContainer(
      ...
      child: Btn.basic(
        ...
        child: Text(
          ...
          style: TextStyle(
            fontSize: 10,
            color: tabOptionIndex == index ? MyTheme.white : MyTheme.dark_font_grey,
            fontWeight: tabOptionIndex == index ? FontWeight.bold : FontWeight.normal,
          ),
        ),
      ),
    );
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                color: _current_slider == index
                                    ? Colors.white // White border for active indicator
                                    : Color(
                                        0xffE62E04,
                              ...
                              color: _current_slider == index
                                  ...
                                  : Colors.transparent, // No fill for inactive indicator
                              boxShadow: [
                                BoxShadow(
          ...
          Container(
            ...
            child: Btn.basic(
              ...
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(6),
                side: BorderSide(
                  color: _isThisSellerFollowed != null && _isThisSellerFollowed! ? MyTheme.green : MyTheme.golden,
                ),
              ),
              onPressed: () {
                if (!is_logged_in.$) {
    ...
    return InkWell(
      onTap: () {
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) {
              return widget.identifier == 'auction' ? AuctionProductsDetails(slug: widget.slug) : ProductDetails(slug: widget.slug);
            },
          ),
        );
      },
