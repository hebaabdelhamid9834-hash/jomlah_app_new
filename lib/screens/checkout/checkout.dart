...
import 'package:active_ecommerce_cms_demo_app/screens/payment_method_screen/cybersource_screen.dart';
import 'package:active_ecommerce_cms_demo_app/screens/payment_method_screen/phonePe/phonePe_screen.dart';
...
class _CheckoutState extends State<Checkout> {
  ...
  String balance() {
    ...
    if (widget.paymentFor == PaymentFor.ManualPayment || widget.paymentFor == PaymentFor.WalletRecharge || widget.paymentFor == PaymentFor.PackagePay || widget.paymentFor == PaymentFor.OrderRePayment) {
      displayValue = SystemConfig.systemCurrency != null ? "${SystemConfig.systemCurrency!.symbol!} ${widget.rechargeAmount.toStringAsFixed(2)}" : widget.rechargeAmount.toStringAsFixed(2);
    } else {
      // For regular orders, use _totalString which comes from cart summary
      displayValue = SystemConfig.systemCurrency != null ? _totalString?.replaceAll(SystemConfig.systemCurrency!.code!, SystemConfig.systemCurrency!.symbol!) : _totalString;
    }
  ...
  fetchAll() async {
    String mode = '';
    setState(() {
      mode = widget.paymentFor != PaymentFor.Order && widget.paymentFor != PaymentFor.ManualPayment ? "wallet" : "order";
    });

    // Fetch payment list first
    var paymentTypeResponseList = await PaymentRepository().getPaymentResponseList(list: widget.list, mode: mode);
    ...
    if (widget.paymentFor == PaymentFor.WalletRecharge || widget.paymentFor == PaymentFor.PackagePay || widget.paymentFor == PaymentFor.OrderRePayment || widget.paymentFor == PaymentFor.ManualPayment) {
      setState(() {
  ...
  fetchSummary() async {
    ...
    if (cartSummaryResponse != null) {
      setState(() {
        ...
        _grandTotalValue = cartSummaryResponse.grand_total_value; // Crucial update
  ...
  onCouponApply() async {
    ...
    var couponApplyResponse = await CouponRepository().getCouponApplyResponse(couponCode);
  ...
  onPressPlaceOrderOrProceed() async {
    if (guest_checkout_status.$ && !is_logged_in.$) {
      Loading.show(context);
      // guest checkout user create response
      var guestUserAccountCreateResponse = await GuestCheckoutRepository().guestUserAccountCreate(widget.guestCheckOutShippingAddress);
      Loading.close();
    ...
    } else if (_selected_payment_method == "manual_payment" && (widget.paymentFor == PaymentFor.ManualPayment || widget.paymentFor == PaymentFor.WalletRecharge || widget.paymentFor == PaymentFor.PackagePay)) {
      Navigator.push(context, MaterialPageRoute(builder: (context) {
  ...
  pay_by_wallet() async {
    loading();
    var orderCreateResponse = await PaymentRepository().getOrderCreateResponseFromWallet(_selected_payment_method_key, _grandTotalValue);
  ...
  pay_by_cod() async {
    loading();
    var orderCreateResponse = await PaymentRepository().getOrderCreateResponseFromCod(_selected_payment_method_key);
    Navigator.of(loadingcontext).pop();
    if (orderCreateResponse.result == false) {
      ToastComponent.showDialog(
        orderCreateResponse.message,
  ...
  pay_by_manual_payment() async {
    loading();
    var orderCreateResponse = await PaymentRepository().getOrderCreateResponseFromManualPayment(_selected_payment_method_key);
                ...
                Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Spacer(),
                        Text(
                          SystemConfig.systemCurrency != null ? _subTotalString!.replaceAll(SystemConfig.systemCurrency!.code!, SystemConfig.systemCurrency!.symbol!) : _subTotalString!,
                          style: TextStyle(color: MyTheme.font_grey, fontSize: 14, fontWeight: FontWeight.w600),
                        ),
                      ],
                    )),
                Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Spacer(),
                        Text(
                          SystemConfig.systemCurrency != null ? _taxString!.replaceAll(SystemConfig.systemCurrency!.code!, SystemConfig.systemCurrency!.symbol!) : _taxString!,
                          style: TextStyle(color: MyTheme.font_grey, fontSize: 14, fontWeight: FontWeight.w600),
                        ),
                      ],
                    )),
                Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Spacer(),
                        Text(
                          SystemConfig.systemCurrency != null ? _shippingCostString.replaceAll(SystemConfig.systemCurrency!.code!, SystemConfig.systemCurrency!.symbol!) : _shippingCostString,
                          style: TextStyle(color: MyTheme.font_grey, fontSize: 14, fontWeight: FontWeight.w600),
                        ),
                      ],
                    )),
                Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Spacer(),
                        Text(
                          SystemConfig.systemCurrency != null ? _discountString!.replaceAll(SystemConfig.systemCurrency!.code!, SystemConfig.systemCurrency!.symbol!) : _discountString!,
                          style: TextStyle(color: MyTheme.font_grey, fontSize: 14, fontWeight: FontWeight.w600),
                        ),
                      ],
                    )),
                ...
                Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Spacer(),
                        Text(
                          SystemConfig.systemCurrency != null ? _totalString!.replaceAll(SystemConfig.systemCurrency!.code!, SystemConfig.systemCurrency!.symbol!) : _totalString!,
            ...
            Align(
              ...
              child: widget.paymentFor == PaymentFor.WalletRecharge || widget.paymentFor == PaymentFor.PackagePay
                  ...
                  : Container(
                      ...
                      height: (widget.paymentFor == PaymentFor.ManualPayment) || (widget.paymentFor == PaymentFor.OrderRePayment) ? 80 : 140,
                      //color: Colors.white,
                      child: Padding(
                        padding: const EdgeInsets.all(16.0),
                        child: Column(
        ...
        children: [
          AnimatedContainer(
            ...
            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(6))
                .copyWith(border: Border.all(color: _selected_payment_method_key == _paymentTypeList[index].payment_type_key ? MyTheme.accent_color : MyTheme.light_grey, width: _selected_payment_method_key == _paymentTypeList[index].payment_type_key ? 2.0 : 0.0)),
              ...
              SizedBox(
                  ...
                  child: Padding(
                      ...
                      child:
                        ...
                        image: _paymentTypeList[index].payment_type == "manual_payment" ? _paymentTypeList[index].image : _paymentTypeList[index].image,
                        fit: BoxFit.fitWidth,
                      ))),
              SizedBox(
                width: 150,
          ...
          Positioned(
            right: 16,
            top: 16,
            child: buildPaymentMethodCheckContainer(_selected_payment_method_key == _paymentTypeList[index].payment_type_key),
          )
        ],
      ),
    );
        ...
        child: Row(
          children: [
            ...
            Padding(
              ...
              child: Text(balance(), style: TextStyle(color: MyTheme.accent_color, fontSize: 14, fontWeight: FontWeight.w600)),
            ),
          ],
        ),
      ),
