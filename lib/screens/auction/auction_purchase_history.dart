...
class _AuctionPurchaseHistoryState extends State<AuctionPurchaseHistory> {
  ...
  final List<DeliveryStatus> _deliveryStatusList = DeliveryStatus.getDeliveryStatusList();
  ...
  init() {
    ...
    for (int x = 0; x < _dropdownPaymentStatusItems!.length; x++) {
      if (_dropdownPaymentStatusItems![x].value!.option_key == _defaultPaymentStatusKey) {
        _selectedPaymentStatus = _dropdownPaymentStatusItems![x].value;
      }
    }

    for (int x = 0; x < _dropdownDeliveryStatusItems!.length; x++) {
      if (_dropdownDeliveryStatusItems![x].value!.option_key == _defaultDeliveryStatusKey) {
  ...
  List<DropdownMenuItem<DeliveryStatus>> buildDropdownDeliveryStatusItems(
    ...
    for (DeliveryStatus item in deliveryStatusList as Iterable<DeliveryStatus>) {
      items.add(DropdownMenuItem(value: item, child: Text(item.name)));
    }
    return items;
  }
  ...
  getPurchaseList() async {
    var purchaseListResponse = await AuctionProductsRepository().getAuctionPurchaseHistory(
      page: _page,
              ...
              slivers: [
                SliverToBoxAdapter(
                  child: _purchaseList.isNotEmpty
                      ...
                      : SizedBox(
                          height: DeviceInfo(context).width! - (AppBar().preferredSize.height + 75),
                          child: Center(
                            child: Text(
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      _purchaseList[index].paymentStatus.toString().replaceRange(
                            0,
                            1,
                            _purchaseList[index].paymentStatus.toString().characters.first.toString().toUpperCase(),
                          ),
                      style: TextStyle(
                        color: _purchaseList[index].paymentStatus == "Paid" ? Colors.green : Colors.red,
                        fontSize: 12,
                        fontWeight: FontWeight.w600,
                      ),
                    ),
      ...
      child: AppBar(
        ...
        flexibleSpace: Padding(
          ...
          child: Column(
            children: [
              Padding(
                padding: MediaQuery.of(context).viewPadding.top > 30 //MediaQuery.of(context).viewPadding.top is the status bar height, with a notch phone it results almost 50, without a notch it shows 24.0.For safety we have checked if its greater than thirty
