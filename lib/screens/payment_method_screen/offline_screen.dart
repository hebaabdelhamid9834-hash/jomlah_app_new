...
class _OfflineState extends State<OfflineScreen> {
  ...
  onPressSubmit() async {
    ...
    if (widget.offLinePaymentFor == PaymentFor.WalletRecharge) {
      var submitResponse = await OfflineWalletRechargeRepository().getOfflineWalletRechargeResponse(
        amount: amount,
        name: name,
        trx_id: trxId,
      ...
      var submitResponse = await OfflinePaymentRepository().getOfflinePaymentSubmitResponse(
  ...
  onPickPhoto(context) async {
    ...
    var imageUpdateResponse = await FileRepository().getSimpleImageUploadResponse(base64Image, fileName);
