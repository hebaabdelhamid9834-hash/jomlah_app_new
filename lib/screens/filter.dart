...
import 'package:active_ecommerce_cms_demo_app/repositories/search_repository.dart';
...
class _FilterState extends State<Filter> {
  ...
  fetchFilteredCategories() async {
    var filteredCategoriesResponse = await CategoryRepository().getFilterPageCategories();
  ...
  init() {
    ...
    for (int x = 0; x < _dropdownWhichFilterItems!.length; x++) {
      if (_dropdownWhichFilterItems![x].value!.option_key == _givenSelectedFilterOptionKey) {
    ...
    _productScrollController.addListener(() {
      if (_productScrollController.position.pixels == _productScrollController.position.maxScrollExtent) {
        setState(() {
          _productPage++;
        });
    ...
    _brandScrollController.addListener(() {
      if (_brandScrollController.position.pixels == _brandScrollController.position.maxScrollExtent) {
        setState(() {
          _brandPage++;
        });
    ...
    _shopScrollController.addListener(() {
      if (_shopScrollController.position.pixels == _shopScrollController.position.maxScrollExtent) {
        setState(() {
          _shopPage++;
        });
  ...
  fetchProductData() async {
    var productResponse = await ProductRepository()
        .getFilteredProducts(page: _productPage, name: _searchKey, sort_key: _selectedSort, brands: _selectedBrands.join(",").toString(), categories: _selectedCategories.join(",").toString(), max: _maxPriceController.text.toString(), min: _minPriceController.text.toString());
  ...
  fetchBrandData() async {
    var brandResponse = await BrandRepository().getBrands(page: _brandPage, name: _searchKey);
  ...
  fetchShopData() async {
    var shopResponse = await ShopRepository().getShops(page: _shopPage, name: _searchKey);
  ...
  List<DropdownMenuItem<WhichFilter>> buildDropdownWhichFilterItems(List whichFilterList) {
    List<DropdownMenuItem<WhichFilter>> items = [];
    for (WhichFilter which_filter_item in whichFilterList as Iterable<WhichFilter>) {
      items.add(
        DropdownMenuItem(
  ...
  Container buildProductLoadingContainer() {
    return Container(
      ...
      color: Colors.white,
      child: Center(
        child: Text(_totalProductData == _productList.length ? AppLocalizations.of(context).no_more_products_ucf : AppLocalizations.of(context).loading_more_products_ucf),
      ),
    );
  }
  ...
  Container buildBrandLoadingContainer() {
    return Container(
      ...
      color: Colors.white,
      child: Center(
        child: Text(_totalBrandData == _brandList.length ? AppLocalizations.of(context).no_more_brands_ucf : AppLocalizations.of(context).loading_more_brands_ucf),
      ),
    );
  }
  ...
  Container buildShopLoadingContainer() {
    return Container(
      ...
      color: Colors.white,
      child: Center(
        child: Text(_totalShopData == _shopList.length ? AppLocalizations.of(context).no_more_shops_ucf : AppLocalizations.of(context).loading_more_shops_ucf),
      ),
    );
  }
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: Stack(fit: StackFit.loose, children: [
          _selectedFilter!.option_key == 'product' ? buildProductList() : (_selectedFilter!.option_key == 'brands' ? buildBrandList() : buildShopList()),
          Positioned(
            top: 10.0,
            left: 0.0,
            right: 0.0,
          ...
          Align(alignment: Alignment.bottomCenter, child: _selectedFilter!.option_key == 'product' ? buildProductLoadingContainer() : (_selectedFilter!.option_key == 'brands' ? buildBrandLoadingContainer() : buildShopLoadingContainer()))
        ]),
      ),
    );
  }
  ...
  Row buildBottomAppBar(BuildContext context) {
    return Row(
      ...
      children: [
        Container(
          decoration: BoxDecoration(color: Colors.white, border: Border.symmetric(vertical: BorderSide(color: MyTheme.light_grey, width: .5), horizontal: BorderSide(color: MyTheme.light_grey, width: 1))),
                              ...
                              return Column(
                                ...
                                children: [
                                  ...
                                  RadioListTile(
                                    ...
                                    title: Text(AppLocalizations.of(context).def_ault_ucf),
        ...
        children: <Widget>[
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: Container(
                child: Padding(
                    padding: MediaQuery.of(context).viewPadding.top > 30 //MediaQuery.of(context).viewPadding.top is the statusbar height, with a notch phone it results almost 50, without a notch it shows 24.0.For safety we have checked if its greater than thirty
                    ...
                    child: TypeAheadField(
                      suggestionsCallback: (pattern) async {
                        ...
                        var suggestions = await SearchRepository().getSearchSuggestionListResponse(query_key: pattern, type: _selectedFilter!.option_key);
                      ...
                      itemBuilder: (context, dynamic suggestion) {
                        ...
                        return ListTile(
                          ...
                          title: Text(
                            suggestion.query,
                            style: TextStyle(color: suggestion.type != "search" ? MyTheme.accent_color : MyTheme.font_grey),
                          ),
                      ...
                      builder: (context, controller, focusNode) {
                        return TextField(
                          ...
                          decoration: InputDecoration(
                              ...
                              suffixIcon: Icon(Icons.search, color: MyTheme.medium_grey),
                            ...
                            child: SizedBox(
                              ...
                              child: TextField(
                                ...
                                decoration: InputDecoration(
                                    ...
                                    focusedBorder: OutlineInputBorder(
                                      borderSide: BorderSide(color: MyTheme.textfield_grey, width: 2.0),
                                      borderRadius: const BorderRadius.all(
                                        Radius.circular(4.0),
                                      ),
                                    ),
    ...
    if (_isProductInitial && _productList.isEmpty) {
      ...
      return RefreshIndicator(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              SizedBox(height: MediaQuery.of(context).viewPadding.top > 40 ? 150 : 135
  ...
  buildBrandScrollableList() {
    if (_isBrandInitial && _brandList.isEmpty) {
      return SingleChildScrollView(child: ShimmerHelper().buildSquareGridShimmer(scontroller: _scrollController));
      ...
      return RefreshIndicator(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              SizedBox(height: MediaQuery.of(context).viewPadding.top > 40 ? 140 : 135
                  //MediaQuery.of(context).viewPadding.top is the statusbar height, with a notch phone it results almost 50, without a notch it shows 24.0.For safety we have checked if its greater than thirty
                  ),
              GridView.builder(
                // 2
                ...
                controller: _scrollController,
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 14, mainAxisSpacing: 14, childAspectRatio: 1),
  ...
  buildShopScrollableList() {
    if (_isShopInitial && _shopList.isEmpty) {
      return SingleChildScrollView(controller: _scrollController, child: ShimmerHelper().buildSquareGridShimmer(scontroller: _scrollController));
          ...
          child: Column(
            children: [
              ...
              GridView.builder(
                ...
                controller: _scrollController,
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 14, mainAxisSpacing: 14, childAspectRatio: 0.7),
                padding: EdgeInsets.only(top: 20, bottom: 10, left: 18, right: 18),
                physics: NeverScrollableScrollPhysics(),
                shrinkWrap: true,
                itemBuilder: (context, index) {
